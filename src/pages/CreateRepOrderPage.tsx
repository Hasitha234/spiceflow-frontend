...
export function CreateRepOrderPage() {
  ...
  useEffect(() => {
    ...
    apiClient.get('/api/v1/sales/master-data/reps?size=2000&sort=id,desc').then(res => setReps(res.data?.content || []));
    apiClient.get('/api/v1/sales/master-data/shops?size=2000&sort=id,desc').then(res => setShopsList(res.data?.content || []));
    apiClient.get('/api/v1/products?size=2000&sort=id,desc').then(res => setProducts(res.data?.content || []));
    apiClient.get('/api/v1/suppliers?size=2000&sort=id,desc').then(res => setSuppliers(res.data?.content || []));
    apiClient.get('/api/v1/warehouses?size=2000&sort=id,desc').then(res => setWarehouses(res.data?.content || []));
